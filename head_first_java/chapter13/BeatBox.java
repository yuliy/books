...
public class BeatBox {
    ...
    public void buildTrackAndStart() {
        ...
        for (int i = 0; i < 16; ++i) {
            ...
            track.add(makeEvent(CMD_CONTROLLER_EVENT, 1, 127, 0, 16));
        }

        track.add(makeEvent(CMD_CHANGE_INSTRUMENT, 9, 1, 0, 15));
        try {
    ...
    public void makeTracks(int[] list) {
        for (int i = 0; i < 16; ++i) {
            int key = list[i];
            if (key != 0) {
                track.add(makeEvent(CMD_NOTE_ON, 9, key, 100, i));
                track.add(makeEvent(CMD_NOTE_OFF, 9, key, 100, i+1));
            }
        }
    }
